...
def train(model, epoch, train_loader, optimizer, versatile=True):
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 15, 0.3)
...
def record_matrix(model, use_loader, log_name):
    ...
    print('End of Matrix Record, Save file in {0}'.format(Para.LOGS_SAVE_FOLDER + log_name))
    print('-' * 99)
    with open(Para.LOGS_SAVE_FOLDER + log_name, 'w+') as f:
        json.dump(result, f)
    return
...
def main_train(model, train_loader, valid_loader, log_name, save_name, lr=Para.learning_rate, epoch_num=Para.epoch_num):
    ...
    for epoch in range(1,  epoch_num + 1):
        ...
        if np.max(v_accu) == v_accu[-1]:
            with open(Para.MODELS_SAVE_FOLDER + save_name, 'wb') as f:
    ...
    with open(Para.LOGS_SAVE_FOLDER + log_name, 'w+') as lf:
